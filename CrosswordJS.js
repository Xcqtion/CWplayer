var Crossword, Puzzle, Grid, Cell;
...
function Cell(letter, number, lColor, cColor) {
  ...
  this.lColor = lColor || "white";
  this.cColor = cColor || "black";
}

Grid = (function() {
  function Grid(answers, makeEmpty){
    // or width, height
    switch(typeof answers){
      case "number":
        this.grid = Array(answers);
        for (var i = this.grid.length - 1; i >= 0; i--) {
          this.grid[i] = Array(makeEmpty);
      ...
      case "array":
        ...
        if (makeEmpty === true) {
          for (var i = this.grid.length - 1; i >= 0; i--) {
            for (var j = this.grid[i].length - 1; j >= 0; j--) {
              var cell = this.grid[i][j].letter
              if(cell.letter !== null) cell.letter = ""
            };
          };
          break;
        } else if (!makeEmpty) {
        ...
        // Don't break if makeEmpty is not a boolean or null so we can error
      default:
        throw "unexpected arguments new Grid()";
        break;
    }
  };
  Grid.prototype = {
    ...
    isBlockingCell: function(x, y){
      return this.isValue(x, y, null)
    },
    isEmptyCell: function(x, y){
      return this.isValue(x, y, "")
    },
    setCell: function(x, y, val){
      return this.grid[x][y].letter = val
    },
    isValue: function(x, y, val){
      return this.grid[x][y].letter === val
    }
  }
...
Puzzle = (function() {
  function Puzzle(lines, clues) {
    this.clues = clues;
    this.answerGrid = new Grid(lines);
    this.userGrid = new Grid(lines.length, lines[0].length);
  };
  Puzzle.prototype = {
